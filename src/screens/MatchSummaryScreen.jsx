...
import { ovDisp, dedupePlayers } from "../utils/helpers";
...
export default function MatchSummaryScreen({
  ...
  // ─────────────────────────────────────────────────────────
  // Player Stats
  // Joker appears in both team arrays — dedupe before rendering
  // to avoid the React duplicate-key warning and doubled rows.
  // ─────────────────────────────────────────────────────────
  ...
  return (
    <div>
      ...
      >
        <div style={{ color: C.textMuted, fontSize: 11, fontWeight: 700, letterSpacing: 1, fontFamily: font }}>
          {tie ? "It's a Tie!" : "Winner"}
        </div>
        <div style={{ color: tie ? C.yellow : won ? C.green : C.red, fontWeight: 900, fontSize: 26, fontFamily: font }}>
          {winner}
        </div>
        {!tie && (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
        <Card style={{ textAlign: "center" }}>
          <div style={{ color: C.textMuted, fontSize: 10, fontWeight: 700, fontFamily: font }}>1st Innings</div>
          <div style={{ color: C.text, fontWeight: 700, fontSize: 13, fontFamily: font }}>{firstTeamName}</div>
          <div style={{ color: C.white, fontWeight: 900, fontSize: 20, fontFamily: font }}>{firstScore}/{firstWickets}</div>
          <div style={{ color: C.textMuted, fontSize: 11, fontFamily: font }}>{ovDisp(firstOver, firstBall)} overs</div>
        </Card>
        <Card style={{ textAlign: "center" }}>
          <div style={{ color: C.textMuted, fontSize: 10, fontWeight: 700, fontFamily: font }}>2nd Innings</div>
          <div style={{ color: C.text, fontWeight: 700, fontSize: 13, fontFamily: font }}>{secondTeamName}</div>
          <div style={{ color: C.white, fontWeight: 900, fontSize: 20, fontFamily: font }}>{secondScore}/{secondWickets}</div>
          <div style={{ color: C.textMuted, fontSize: 11, fontFamily: font }}>{ovDisp(secondOver, secondBall)} overs</div>
        </Card>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
        {[
          { icon: "🏏", title: "Best Bat",  name: bestBat?.name  || "-", stat: `${bestBat?.runs    ?? 0} runs`, color: C.green },
          { icon: "⚡", title: "Best Bowl", name: bestBowl?.name || "-", stat: `${bestBowl?.wickets ?? 0} wkts`, color: C.red   },
          { icon: "🎯", title: "Target",    name: required,               stat: won ? "Achieved" : "Not achieved", color: won ? C.green : C.red },
        ].map(({ icon, title, name, stat, color }) => (
          <Card key={title} style={{ padding: 12 }}>
            <div style={{ fontSize: 18 }}>{icon}</div>
            <div style={{ color: C.textMuted, fontSize: 10, fontWeight: 700, fontFamily: font }}>{title}</div>
            <div style={{ color, fontWeight: 700, fontSize: 13, fontFamily: font }}>{name}</div>
            <div style={{ color: C.textMuted, fontSize: 11, fontFamily: font }}>{stat}</div>
          </Card>
        ))}
      </div>
